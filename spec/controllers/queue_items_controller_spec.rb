...
describe QueueItemsController do
  ...
  describe "POST update_queue" do
    context "with valid input" do
      let (:wk) { Fabricate(:user) }
      let (:video) { Fabricate(:video) }
      let (:queue_item1) { Fabricate(:queue_item, user: wk, position: 1, video: video) }
      let (:queue_item2) { Fabricate(:queue_item, user: wk, position: 2, video: video) }
    ...
    context "with invalid input" do
      ...
      it "does not change the queue items that do not belong to the user" do
        ...
        wkm = Fabricate(:user)
        video = Fabricate(:video)
        queue_item1 = Fabricate(:queue_item, user: wkz, position: 1, video: video)
        queue_item2 = Fabricate(:queue_item, user: wkm, position: 2, video: video)
    ...
    context "with unauthenticated user" do
      it_behaves_like "require sign in" do
        let(:action) { post :update_queue, queue_items: 1 }
      end
    end
  end
end